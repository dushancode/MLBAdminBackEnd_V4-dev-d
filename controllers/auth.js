...
exports.sendResponse = async (req, res) => {
  try {
    const pictures = await uploadImage(req.file);
    console.log("pictures", pictures);
    let awsUrl;
    // BUCKET_NAME = 'bckt-mylibrary-testing'
    // BUCKET_REGION = 'ap-south-1'
    // https://bckt-mylibrary-testing.s3.ap-south-1.amazonaws.com/
    if (pictures) {
      awsUrl = `https://${process.env.BUCKET_NAME}.s3.${process.env.BUCKET_REGION}.amazonaws.com/${pictures}`;
    }
    if (awsUrl) {
